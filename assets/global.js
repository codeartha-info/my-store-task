...
class VariantSelects extends HTMLElement {
  ...
  connectedCallback() {
    this.addEventListener('change', (event) => {
      if (event.target.id !== "add-name" && event.target.name !== "properties[name]" && event.target.name !== "properties[color]" && event.target.name !== "properties[font-family]") {
      ...
      } else if (event.target.id === "add-name") {
        event.target.closest(".embroidered-name-container").querySelector(".main-embroidered").classList.toggle("hidden");
      }
    });
  }
